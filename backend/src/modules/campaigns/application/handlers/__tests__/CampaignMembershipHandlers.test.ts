...
function createMembershipRepository(): CampaignMembershipRepository {
  return {
    findActiveMemberByUserId: vi.fn().mockResolvedValue(null),
...
describe("campaign membership handlers", () => {
  it("creates invited campaign member invitation", async () => {
    ...
    const handler = new InviteCampaignMemberHandler(campaignRepository, membershipRepository);
  ...
  it("blocks removing owner without ownership transfer", async () => {
    ...
    const handler = new RemoveCampaignMemberHandler(campaignRepository, membershipRepository);
