...
import { createItemCatalogRouter } from "@api/routes/item-catalog.routes";
...
export function createApiRouter(options: ApiRoutesOptions): RequestHandler {
  ...
  router.use(
    ...
    createCampaignsRouter(
      ...
      options.campaignFightTrackerController,
  ...
  router.use(
    "/api/v1/item-catalog",
    createItemCatalogRouter(
      options.itemCatalogController,
      options.authMiddleware,
    ),
  );
